...
void loci_createMeshModel(ecs_entity_t entity,
                          ...
                          vec3 ambient, float roughness, float metallic, float reflection)
{
    ...
    for (cgltf_size i = 0; i < data->textures_count; i++)
    {
        ...
        {
            LOCI_LOGE("createMeshModel", "gltfTexture->image", "NULL")
        }
        if(gltfTexture->image->uri == NULL)
        {
            LOCI_LOGE("createMeshModel", "gltfTexture->image->uri", "NULL")
        }
